...
gxf_result_t EncoderRequestReceptiveSchedulingTerm::registerInterface(
...
gxf_result_t EncoderRequestReceptiveSchedulingTerm::check_abi(int64_t timestamp,
  gxf::SchedulingConditionType* type, int64_t* target_timestamp) const {
  auto maybe_is_accepting_request = encoder_impl_.get()->isAcceptingRequest();
  if (!maybe_is_accepting_request) {
    GXF_LOG_ERROR("Encoder isAcceptingRequest had unexpected return");
    return GXF_FAILURE;
  }
...
gxf_result_t EncoderRequestReceptiveSchedulingTerm::onExecute_abi(int64_t dt) {
  return GXF_SUCCESS;
}
