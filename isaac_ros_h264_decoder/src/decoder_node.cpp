  ...
  output_encoding_(declare_parameter<std::string>("output_encoding", "rgb8"))
{
  ...
  // Accept GPU-backed compressed buffers; from_input_buffer promotes CPU buffers as needed.
...
void DecoderNode::compressed_callback(const sensor_msgs::msg::CompressedImage::ConstSharedPtr & msg)
{
  ...
  if (!decoder_->decode_frame(data_ptr, size, timestamp_ns, msg->header.frame_id, cuda_stream_)) {
...
void DecoderNode::on_decoded_frame(DecodedFrame && frame)
{
  ...
  auto output = vpi_conversions::allocate_image_msg(width, height, output_encoding_);
  const uint32_t out_step = output->step;
  ...
  // cuda_stream_ before the message is published.
  {
    auto output_wrapper = vpi_conversions::from_output_image_msg(*output, cuda_stream_);
    uint8_t * dst = output_wrapper.data_ptr();
    ...
    if (output_encoding_ == "rgb8") {
      ...
      VPIStatus vpi_err = codec::CreateOrSetWrapper(vpi_input_, nv12_data);
      if (vpi_err == VPI_SUCCESS) {
        vpi_err = vpiSubmitConvertImageFormat(
          vpi_stream_, VPI_BACKEND_CUDA, vpi_input_, output_wrapper.image(), nullptr);
      }
      if (vpi_err == VPI_SUCCESS) {
        vpi_err = vpiStreamSync(vpi_stream_);
      }
      if (vpi_err != VPI_SUCCESS) {
      ...
      if (err == cudaSuccess && output_encoding_ == "nv12") {
        ...
        // It is laid out contiguously after the Y plane in the output buffer.
